...
var arr = [2, -4, -5, 6, 7, 10];
let positiveArray = arr.filter((element) => {
    if (element > 0) {
        return element;
    }
});
console.log("remove negative number from an array of numbers", positiveArray);
...
const fruitNames = ["apple", "banana", "cherry", "date", "grapes"];
...
//a new array where each temperature is converted to Fahrenheit 
//using the formula (Celsius * 9/5) + 32.
const celsiusTemp = [0, 10, 20, 30, 40];
let fahrenheitTemp = celsiusTemp.map((temp) => { return temp * (9 / 5) + 32; });
...
const arrOfNumbers = [3, 6, 9, 12, 15, 18];
let doubledValues = arrOfNumbers.filter((element) => {
    return element % 2 != 0;
}).map((oddNum) => {
